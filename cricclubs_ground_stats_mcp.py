...
An MCP (Model Context Protocol) tool that takes a CricClubs series URL
and returns match duration statistics aggregated by ground.

This allows Claude Code to fetch cricket match stats on demand when you ask
questions like: "Get ground stats for https://www.cricclubs.com/ARCL/..."

Usage:
    As MCP server: python cricclubs_ground_stats_mcp.py
    The server communicates via stdio with Claude Code.

Requires:
    pip install "mcp[cli]" cloudscraper
"""
...
def fetch_url(url: str) -> str:
    """Fetch a URL using cloudscraper (handles Cloudflare)."""
...
def parse_url(series_url: str) -> tuple[str, str, str]:
    ...
    Example input:
        https://www.cricclubs.com/ARCL/listMatches.do?league=321&clubId=992

    Returns:
        ("https://www.cricclubs.com/ARCL", "321", "992")
    """
...
def fetch_fixtures(base_url: str, league_id: str, club_id: str) -> list[dict]:
    """
    Fetch the fixtures page and extract all matches.

    Returns a list of match dictionaries with match_id, date, team1, team2.
    """
...
def fetch_match_info(base_url: str, club_id: str, match_id: str) -> dict:
    """Fetch and parse the info page for a single match."""
...
def get_ground_stats(series_url: str) -> str:
    ...
    Returns:
        A markdown table showing match count, avg/min/max duration per ground.
    """
