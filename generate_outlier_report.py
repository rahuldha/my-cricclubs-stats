"""
Outlier Match Report Generator

Creates a PDF report of matches that exceeded 140 minutes.
This helps identify matches that ran long due to:
- Late starts
- Long innings breaks
- Full overs played
...
def safe_int(s):
    """Convert a string like '123 min' to integer, returning 0 if invalid."""
...
def parse_time(t):
    """
    Convert time string like '10:30 AM' to minutes since midnight.
    ...
    h = int(parts[0])
    m = int(parts[1].replace('AM', '').replace('PM', '').strip())
...
def scheduled_start(actual_mins):
    """
    Estimate scheduled start time (rounds down to nearest 30 minutes).

    If a match started at 10:43 AM, the scheduled time was likely 10:30 AM.
    """
...
# Determine last match of day per ground (these are expected to run longer)
...
# Filter to outliers (matches > 140 minutes)
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, f'ARCL Match Duration Outlier Report (>{OUTLIER_THRESHOLD} min)', align='C', new_x='LMARGIN', new_y='NEXT')
...
pdf.add_page()
pdf.set_font('Helvetica', 'B', 12)
pdf.cell(0, 8, f'All Outlier Matches (>{OUTLIER_THRESHOLD} min) - Sorted by Duration', new_x='LMARGIN', new_y='NEXT')
pdf.ln(2)

# Column widths
col_w = {
    '#': 8, 'Dur': 12, 'Break': 13, 'Delay': 13, 'Total': 13,
    'Sched': 17, 'Actual': 17, 'Match': 68, 'Date': 22,
    'Ground': 55, 'Last': 12, 'Max Ov': 14,
}
...
for i, m in enumerate(long, 1):
    ...
    # Highlight extreme outliers (>170 min) in red
    if m['dur'] > 170:
...
# Save PDF
output_path = 'arcl_outlier_matches_report.pdf'
